...
// load hybridauth
// require_once( dirname(__FILE__) . "/vendors/hybridauth/Hybrid/Auth.php" );   // OBS
...
if (isset($_GET["provider"]) && !isset($_GET["redirect_to_provider"])) {
    ...
    <script>
        setTimeout(function () {
            window.location.href = window.location.href + "&redirect_to_provider=true"
        }, 750);
    </script>
    <?php
    die();
            ...
            if ($user_profile->photoURL) {
                ...
                foreach ($sizes as $size => $dimensions) {
                    $image = get_resized_image_from_existing_file(
                            $user_profile->photoURL, $dimensions[0], $dimensions[1], $dimensions[2]
                    );
